// src/app/seproc/pages/admin-institucion-dashboard/admin-institucion-dashboard.component.ts
...
import {
  ...
  UsuarioUpsert,
  CambiarPassword,
} from '../../../core/models/admin-institucion.model';
import { AdminInstitucionService } from '../../../core/services/admin-institucion.service';
...
type AccionUsuarioPendiente = 'APROBAR' | 'RECHAZAR';
...
@Component({
  ...
  templateUrl: './admin-institucion-dashboard.component.html',
  styleUrl: './admin-institucion-dashboard.component.css',
})
export class AdminInstitucionDashboardComponent implements OnInit {
  private readonly fb = inject(NonNullableFormBuilder);
  private readonly route = inject(ActivatedRoute);
  private readonly router = inject(Router);
  private readonly titleService = inject(Title);
  private readonly adminService = inject(AdminInstitucionService);
  private readonly destroyRef = inject(DestroyRef);
  private readonly cdr = inject(ChangeDetectorRef);
  private versionFotoPerfil = Date.now();

  readonly vistasValidas: AdminInstitucionView[] = [
    'solicitudes',
    'proyectos',
  ...
  readonly rolesDisponibles = [
    ...
    { value: 'contratista', label: 'Constructor' },
    { value: 'direccion', label: 'Director' },
    { value: 'central', label: 'Central' },
    { value: 'administrador', label: 'Administrador' },
  ];

  perfil: PerfilAdminInstitucion | null = null;

  vista: AdminInstitucionView = 'solicitudes';
  ...
  estadoPendienteCambio: EstadoProyecto | any = null;
  ...
  solicitudDetalle: SolicitudDetalle | null = null;
  documentacionSolicitud: DocumentacionInicial | null = null;
  ...
  proyectoDetalle: ProyectoDetalle | null = null;
  ...
  detalleEtapa: DetalleEtapa | null = null;
  historialEtapa: HistorialEtapa[] = [];

  documentacionProyecto: DocumentacionInicial | null = null;
  modalDocumentacionAbierto = false;

  usuarios: UsuarioAdmin[] = [];
  usuariosPendientes: UsuarioAdmin[] = [];
  rolesPendientes: Record<number, string> = {};

  modalConfirmarUsuarioPendienteAbierto = false;
  modalResultadoUsuarioPendienteAbierto = false;

  usuarioPendienteSeleccionado: UsuarioAdmin | null = null;
  accionUsuarioPendiente: AccionUsuarioPendiente | null = null;
  rolUsuarioPendiente = '';

  procesandoUsuarioPendiente = false;

  tituloResultadoUsuarioPendiente = '';
  mensajeResultadoUsuarioPendiente = '';
  tipoResultadoUsuarioPendiente: 'success' | 'error' = 'success';
  ...
  ngOnInit(): void {
    ...
    this.titleService.setTitle(
      abreviacionGuardada
        ? `${abreviacionGuardada.toUpperCase()} | Modulo Administrador`
        : 'SEPROC | Modulo Administrador',
    );
  ...
  private esVistaValida(view: string): view is AdminInstitucionView {
    return this.vistasValidas.includes(view as AdminInstitucionView);
  }

  esVistaUsuarios(view: AdminInstitucionView = this.vista): boolean {
    return view.startsWith('usuarios-');
  }

  cambiarVista(view: AdminInstitucionView): void {
      ...
      .subscribe({
        next: (perfil) => {
          ...
          if (abreviacion) {
            ...
            this.titleService.setTitle(`${abreviacion.toUpperCase()} | Modulo Administrador`);
          } else {
            this.titleService.setTitle('SEPROC | Modulo Administrador');
          }
  ...
  seleccionarEstadoSolicitud(estado: EstadoSolicitud): void {
    this.estadoSolicitud = estado;
    this.cargarSolicitudes();
  }

  get solicitudesFiltradas(): SolicitudResumen[] {
  ...
  abrirSolicitud(idSolicitud: number): void {
    ...
    forkJoin({
      detalle: this.adminService.obtenerDetalleSolicitud(idSolicitud),

      documentacion: this.adminService
  ...
  seleccionarEstadoProyecto(estado: EstadoProyecto): void {
    this.estadoProyecto = estado;
    this.cargarProyectos();
  }

  get proyectosFiltrados(): ProyectoResumen[] {
    const texto = this.normalizar(this.busqueda);
  ...
  seleccionarNuevoEstado(estado: EstadoProyecto): void {
    this.menuEstadoAbierto = false;
  ...
  solicitarCorreccion(documento: DocumentoInicial): void {
  ...
  aprobarDocumento(documento: DocumentoInicial): void {
    if (!documento.idDocumento) {
      return;
    }
  ...
  guardarUsuario(): void {
    ...
    const payload: UsuarioUpsert = this.usuarioForm.getRawValue();
    ...
    const peticion = esCreacion
      ...
      : this.adminService.actualizarUsuario(
  ...
  private obtenerRolDeVista(): string {
    const roles: Partial<Record<AdminInstitucionView, string>> = {
  ...
  aprobarUsuarioPendiente(usuario: UsuarioAdmin): void {
    const rol = this.rolesPendientes[usuario.idUsuario]?.trim();

    if (!rol) {
      this.abrirResultadoUsuarioPendiente(
        'Rol requerido',
        `Selecciona un rol antes de aprobar a ${usuario.nombre} ${usuario.apellido}.`,
        'error',
      );
      return;
    }

    this.usuarioPendienteSeleccionado = usuario;
    this.accionUsuarioPendiente = 'APROBAR';
    this.rolUsuarioPendiente = rol;
    this.modalConfirmarUsuarioPendienteAbierto = true;
  }

  rechazarUsuarioPendiente(usuario: UsuarioAdmin): void {
    this.usuarioPendienteSeleccionado = usuario;
    this.accionUsuarioPendiente = 'RECHAZAR';
    this.rolUsuarioPendiente = '';
    this.modalConfirmarUsuarioPendienteAbierto = true;
  }

  get nombreRolUsuarioPendiente(): string {
    const rolEncontrado = this.rolesDisponibles.find(
      (rol) => rol.value === this.rolUsuarioPendiente,
    );

    return rolEncontrado?.label ?? this.rolUsuarioPendiente;
  }

  confirmarAccionUsuarioPendiente(): void {
    const usuario = this.usuarioPendienteSeleccionado;
    const accion = this.accionUsuarioPendiente;

    if (!usuario || !accion || this.procesandoUsuarioPendiente) {
      return;
    }

    if (accion === 'APROBAR' && !this.rolUsuarioPendiente) {
      this.modalConfirmarUsuarioPendienteAbierto = false;
      this.limpiarSeleccionUsuarioPendiente();
    ...
    this.procesandoUsuarioPendiente = true;

    const peticion =
      accion === 'APROBAR'
        ? this.adminService.aprobarUsuarioPendiente(
          usuario.idUsuario,
          this.rolUsuarioPendiente,
        )
        : this.adminService.rechazarUsuarioPendiente(usuario.idUsuario);

    peticion
      .pipe(
        finalize(() => {
          this.procesandoUsuarioPendiente = false;
          this.cdr.markForCheck();
        }),
        takeUntilDestroyed(this.destroyRef),
      )
      .subscribe({
        next: (respuesta) => {
          ...
          const mensajePredeterminado = esAprobacion
            ? `${usuario.nombre} ${usuario.apellido} fue aprobado correctamente.`
            : `La solicitud de ${usuario.nombre} ${usuario.apellido} fue rechazada correctamente.`;
          ...
          this.abrirResultadoUsuarioPendiente(
            titulo,
            respuesta.mensaje || mensajePredeterminado,
            'success',
          );
  ...
  cancelarAccionUsuarioPendiente(): void {
    if (this.procesandoUsuarioPendiente) {
      return;
    }
  ...
  cerrarResultadoUsuarioPendiente(): void {
    this.modalResultadoUsuarioPendienteAbierto = false;
    this.tituloResultadoUsuarioPendiente = '';
    this.mensajeResultadoUsuarioPendiente = '';
  }

  private abrirResultadoUsuarioPendiente(
    titulo: string,
    mensaje: string,
    tipo: 'success' | 'error',
  ): void {
    this.tituloResultadoUsuarioPendiente = titulo;
    this.mensajeResultadoUsuarioPendiente = mensaje;
    this.tipoResultadoUsuarioPendiente = tipo;
    this.modalResultadoUsuarioPendienteAbierto = true;
    this.cdr.markForCheck();
  }

  private limpiarSeleccionUsuarioPendiente(): void {
    this.usuarioPendienteSeleccionado = null;
    this.accionUsuarioPendiente = null;
    this.rolUsuarioPendiente = '';
  }
  ...
  get fotoPerfil(): string {
    ...
    const url = this.adminService.resolverRecurso(fotoUrl);
  ...
  get logoEmpresa(): string {
    ...
    if (!logoInvalido) {
      return this.adminService.resolverRecurso(logoUrl);
    }
  ...
  recurso(url?: string | null): string {
    return this.adminService.resolverRecurso(url);
  }
  ...
  get subtituloSeccion(): string {
    switch (this.vista) {
      ...
      case 'pendientes':
        return 'Aprueba o rechaza solicitudes de acceso';
